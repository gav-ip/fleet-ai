...
selected_vehicle = st.sidebar.selectbox("Select a Vehicle", latest_data['vehicle_id'].unique())
...
st.line_chart(vehicle_data.set_index('timestamp')[['engine_rpm', 'coolant_temp', 'fuel_trim', 'tire_pressure']])
...
if not selected_vehicle_location.empty:
    folium.Marker(
        [selected_vehicle_location.iloc[0]['lat'], selected_vehicle_location.iloc[0]['lon']],
